...
public class Triangle extends AbstractShape
{
    ...
    */
    ...
    /**
     * Check the clicked point is in the triangle bounding box
     * @param clicked point
     * @return boolean of clicked point is in bounding box
     */
    public boolean inShape(int x,int y)
    {
        int xSet[] = {vertices.get(0).x,vertices.get(1).x,vertices.get(2).x};   //set arraylist of x verticle
        int ySet[] = {vertices.get(0).y,vertices.get(1).y,vertices.get(2).y};   //set arraylist of y verticle
        Arrays.sort(xSet);  // sort array to order x set
        Arrays.sort(ySet);  // sort array to order y set
        int xMin = xSet[0];                 //get the minimum of x
        int xMax = xSet[xSet.length-1];     //get the maximum of x
        int yMin = ySet[0];                 //get the minimum of y
        int yMax =  ySet[ySet.length-1];    //get the maximum of y
